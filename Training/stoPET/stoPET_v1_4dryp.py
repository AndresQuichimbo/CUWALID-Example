...
def stoPET_wrapper_regional(startyear, endyear, latval_min, latval_max, lonval_min, lonval_max,
    ...
    stopet = future_pet_ts_generate_regional(startyear, endyear, latval_min,latval_max, lonval_min,lonval_max,
                                             ...
                                             slope_vals, mcont_vals,ens_num, datapath, outputpath, tempAdj, deltat, dpetdt, smoother, N, npass)
...
def future_pet_ts_generate_regional(startyear, endyear, latval_min,latval_max, lonval_min,lonval_max, lats, lons, locname,
    ...
    for i in range(0,len(years)):
        ...
        for month in range(1,13):
            ...
            noise_all = np.where(noise_all > 1e2, 1., noise_all)
            
            # randoumly preterb the data to generate month lenght value
            for i in range(0,m[month-1]):
                # generate a random value for each day
                # minumum and maximum are seleced from the peak hour value of the ratio 
                noise_ori = noise_all[i,:,:]
                noise = smoothing_func(noise_ori, smoother, N, npass)
                spet = noise * res0
...
def seasonal_pet_for_dryp(outputpath, locname, number_ensm, tempAdj, startyear, endyear, startdate, enddate, seasonswitch):
    ...
    for i in range(0,number_ensm):
      ...
      for j in range(0,len(years)):
        ...
        stopet4dryp(filepath, fname1, seasonswitch, startdate, enddate)
        fname2 = '%s_%s_AdjstoPET.nc'%(year, tempAdj)
        stopet4dryp(filepath, fname2, seasonswitch, startdate, enddate)
...
def stopet4dryp(filepath, fname, seasonswitch, startdate, enddate):
    ...
    seasonal_nc_write(new_pet, lats, lons, 'pet', tunits, filename, startdate, int(year))
...
def smoothing_func(raw_data, smoother, N, npass):
  """
  This function smooth the spatial variability of the noise ratio
  used to randomize the PET.
  """
  # Gaussian Smoother
  if smoother == 'Gaussian':
    data = mpcalc.smooth_gaussian(raw_data, N)
  # 5-point smoother
  elif smoother == '5Npoint':
    data = mpcalc.smooth_n_point(raw_data, 5, npass)
  # 9-point smoother
  elif smoother == '9Npoint':
    data = mpcalc.smooth_n_point(raw_data, 9, npass)
  else:
    data = raw_data
  return data
